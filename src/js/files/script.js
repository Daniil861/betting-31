...
export function initStartData() {
	if (sessionStorage.getItem('money')) {
		...
		sessionStorage.setItem('money', 66000);
		writeScore();
	}
	...
	if (document.querySelector('.bet-box__score')) {
...
export function drawCurrentBall() {
	const ballItem = document.querySelector('.main__image-box img');
	const cuurentBall = +sessionStorage.getItem('current-subject');
	ballItem.setAttribute('src', `img/game/ball-${cuurentBall}.png`);
}
